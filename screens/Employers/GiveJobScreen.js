...
export default function GiveJobScreen({ navigation }) {
  ...
  const handlePostJob = async () => {
    ...
    const res = await API.get("/auth/profile-status", {
  ...
  const renderJob = ({ item }) => (
    ...
    >
      ...
      <View style={styles.jobMeta}>
        <Text style={styles.metaText}>{`💰 ₹${item.amount ?? 0}`}</Text>
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <TouchableOpacity onPress={() => navigation.navigate("Settings", { userType: 'employer' })}>
      ...
      <TouchableOpacity style={styles.fab} onPress={handlePostJob}>
